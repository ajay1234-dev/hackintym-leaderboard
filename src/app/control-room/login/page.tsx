...
export default function CinematicLogin() {
  ...
  const handleSubmitSequence = async (e: React.FormEvent) => {
    ...
    if (step === "FIREBASE_AUTH") {
      try {
        ...
        const adminEmail =
          process.env.NEXT_PUBLIC_ADMIN_EMAIL || "admin@hackintym.com";
      ...
      const secret =
        process.env.NEXT_PUBLIC_CONTROL_SECRET || "EVOLUTION_ARENA_2603";
  ...
  const cHoverBorder = isSuccess
    ? "hover:border-[#39ff14]"
    : "hover:border-red-500";
  const cBgSubtle = isSuccess ? "bg-[#39ff14]/10" : "bg-red-500/10";
  const cHoverBg = isSuccess ? "hover:bg-[#39ff14]/20" : "hover:bg-red-500/20";
  const cShadow = isSuccess
    ? "shadow-[0_0_20px_rgba(57,255,20,0.3)]"
    : "shadow-[0_0_20px_rgba(239,68,68,0.3)]";
  const cHoverShadow = isSuccess
    ? "hover:shadow-[0_0_30px_rgba(57,255,20,0.3)]"
    : "hover:shadow-[0_0_30px_rgba(239,68,68,0.3)]";
  const cGlow = isSuccess
    ? "0 0 15px rgba(57,255,20,0.5)"
    : "0 0 15px rgba(239,68,68,0.5)";
  const cFocusBorder = isSuccess
    ? "focus:border-[#39ff14]"
    : "focus:border-red-500";
  const cFocusShadow = isSuccess
    ? "focus:shadow-[0_0_20px_rgba(57,255,20,0.2)]"
    : "focus:shadow-[0_0_20px_rgba(239,68,68,0.2)]";
  const cScanline = isSuccess
    ? "bg-[repeating-linear-gradient(0deg,transparent,transparent_2px,#39ff14_3px)]"
    : "bg-[repeating-linear-gradient(0deg,transparent,transparent_2px,#ef4444_3px)]";

  return (
    <main
      className={`min-h-screen flex items-center justify-center p-4 bg-zinc-950 ${cText} overflow-hidden transition-all duration-300 ${
        step === "GLITCH_TRAP"
          ? "bg-black text-red-500"
          : "bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-zinc-900 via-zinc-950 to-black"
      }`}
    >
      {/* Background Decor (Grid Lines) */}
      <div
        className={`absolute inset-0 pointer-events-none opacity-[0.03] bg-[linear-gradient(currentColor_1px,transparent_1px),linear-gradient(90deg,currentColor_1px,transparent_1px)] bg-[size:40px_40px] [mask-image:radial-gradient(ellipse_at_center,black,transparent_80%)] transition-colors duration-500 mix-blend-screen`}
      ></div>
      ...
      <AnimatePresence mode="wait">
        ...
        {step === "GLITCH_TRAP" && (
          ...
          >
            ...
            <h1 className="text-4xl md:text-7xl font-black text-red-500 mb-8 uppercase tracking-tighter text-center">
              Access Denied
            </h1>
              ...
              <AnimatePresence>
                ...
                {glitchLogs.length === 5 && (
                  <motion.div
                    ...
                    className="mt-6 text-center text-red-500 font-black text-2xl md:text-3xl animate-pulse"
                  >
        ...
        {step !== "GLITCH_TRAP" && (
          ...
          >
            {/* Scanline overlay */}
            <div
              className={`absolute inset-0 rounded-3xl opacity-10 pointer-events-none transition-all duration-500 mix-blend-overlay ${cScanline}`}
            ></div>

            <div className="flex flex-col items-center mb-10 relative">
              <div
                className={`w-16 h-16 rounded-2xl ${cBgSubtle} border ${cBorder} flex items-center justify-center mb-6 ${cShadow} transition-all duration-500`}
              >
                {isSuccess || step === "AUTHENTICATING" ? (
                  <Unlock className={`w-8 h-8 ${cText} transition-colors`} />
                ) : step === "FIREBASE_AUTH" ? (
                  <Lock className={`w-8 h-8 ${cText} transition-colors`} />
                ) : (
                  <Key className={`w-8 h-8 ${cText} transition-colors`} />
                )}
              </div>
              <h2
                className={`text-sm font-bold tracking-[0.4em] uppercase opacity-70 mb-2 text-center transition-opacity ${cText}`}
              >
                Prime Directive Override
              </h2>
              <h1
                className={`text-3xl md:text-4xl font-black uppercase tracking-tighter text-center transition-all ${cText}`}
                style={{ textShadow: cGlow }}
              >
                Control Room
              </h1>
            </div>
            ...
            {step === "AUTHENTICATING" && (
              ...
              >
                <Fingerprint
                  className={`w-16 h-16 animate-pulse mb-6 opacity-80 ${cText}`}
                />
                <div
                  className={`text-lg md:text-xl font-mono uppercase tracking-widest text-center animate-pulse drop-shadow-[0_0_10px_currentColor] ${cText}`}
                >
                  Verifying Internal Protocol...
                </div>
                <div
                  className={`mt-4 text-[10px] uppercase tracking-widest opacity-50 ${cText}`}
                >
                  Synchronizing socket keys
                </div>
              </motion.div>
            )}
            ...
            {(step === "FIREBASE_AUTH" || step === "PASSPHRASE") && (
              <form onSubmit={handleSubmitSequence} className="space-y-6">
                <div className="space-y-2">
                  <label
                    className={`text-xs font-bold uppercase tracking-[0.2em] opacity-80 ml-1 transition-colors ${cText}`}
                  >
                    {step === "FIREBASE_AUTH"
                      ? "Phase 1: Admin Credentials"
                      : "😂😂 Not so fast final Access Verification "}
                  </label>
                  <div className="relative group">
                    <ChevronRight
                      className={`absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 transition-colors opacity-50 group-focus-within:opacity-100 ${cText}`}
                    />
                    <input
                ...
                >
                  <div
                    className={`absolute inset-0 bg-gradient-to-r from-transparent via-current to-transparent opacity-20 -translate-x-[200%] group-hover:animate-[shimmer_2s_infinite]`}
                  ></div>
