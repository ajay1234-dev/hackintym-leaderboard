...
const ActiveEffectBadge = ({ effect }: { effect: ActiveEffect }) => {
  ...
  const isLegacyString = !hasLucideIcon && (effect.icon || '').length > 2 && /^[a-zA-Z]+$/.test(effect.icon || '');
  const renderEmoji = isLegacyString ? '✨' : (effect.icon || '✨');
  ...
  const formatTime = (secs: number) => {
    ...
    return `${mins.toString().padStart(2, '0')}:${splitSecs.toString().padStart(2, '0')}`;
  };

  return (
    <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-zinc-900 border border-zinc-700/50 hover:bg-zinc-800 transition-colors cursor-default select-none relative group">
      {IconComp ? <IconComp className="w-3 h-3 text-zinc-400" /> : <span className="text-[10px] drop-shadow-sm pointer-events-none select-none">{renderEmoji}</span>}
      <span className="text-[10px] font-bold text-zinc-300">
        {effect.isPending ? `${textToDisplay} (Ready)` : timeLeft !== null ? `${textToDisplay} (${formatTime(timeLeft)})` : textToDisplay}
      </span>
      <div className="absolute opacity-0 group-hover:opacity-100 transition-opacity bg-black border border-zinc-800 text-xs px-2 py-1 rounded text-white bottom-full mb-2 left-1/2 -translate-x-1/2 whitespace-nowrap z-50 pointer-events-none">
        {effect.effect.replace('_', ' ').toUpperCase()} effect active
      </div>
    </div>
  );
};
...
function LeaderboardRow({ team, index, totalRows, renderCard, revealPhase }: { team: TeamWithScore, index: number, totalRows: number, renderCard: (id: string, isUsed: boolean, i: number, isActive?: boolean) => React.ReactNode, revealPhase: RevealPhase }) {
  ...
  return (
    ...
    >
      ...
      <div className="col-span-1 flex justify-center">
        <span className={`text-base sm:text-lg md:text-2xl font-black ${rankClass}`}>
          #{index + 1}
        </span>
      </div>
      ...
      <div className="col-span-4 sm:col-span-3 md:col-span-2 text-left pl-1">
        <h3 className="text-[11px] sm:text-[13px] md:text-base font-bold text-white truncate transition-colors">
          {team.teamName}
        </h3>
      </div>
      ...
      <div className="col-span-3 sm:col-span-2 md:col-span-3 flex items-center gap-1 sm:gap-1.5 flex-nowrap overflow-visible text-left pl-1">
          {(() => {
            ...
            return (
              <>
                 {allCards.map(c => renderCard(c.id, c.isUsed, c.index, team.activeEffects?.some(e => e.sourceCardId === c.id)))}
...
export default function Leaderboard() {
  ...
  const renderCard = (cardId: string, isUsed: boolean, i: number, isActive: boolean = false) => {
    ...
    if (isUsed) {
       ...
       finalStyle = typeColor + ' animate-pulse ring-2 ring-white/50 scale-[1.05] shadow-[0_0_15px_currentColor]';
    }
    ...
    return (
      ...
      >
         ...
         {IconComponent ? (
            <IconComponent className={`w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5 ${isUsed ? 'opacity-40' : ''}`} />
         ) : (
            <span className={`text-[12px] sm:text-[16px] md:text-[18px] leading-none drop-shadow-md select-none pointer-events-none scale-110 ${isUsed ? 'opacity-40 grayscale blur-[0.5px]' : ''}`}>{renderEmoji}</span>
         )}
      </motion.div>
    );
  };
    ...
    <div className="flex flex-col h-full rounded-2xl glass-panel border border-zinc-800 overflow-hidden relative shadow-[0_0_50px_rgba(0,0,0,0.5)] bg-zinc-950/90">
      ...
      >
        ...
        <div className="grid grid-cols-12 md:grid-cols-16 gap-1 md:gap-2 p-2 sm:p-4 border-b border-zinc-800 bg-zinc-900/80 text-[8px] sm:text-[9px] md:text-xs font-bold uppercase tracking-wider text-zinc-400">
          ...
          <div className="col-span-4 sm:col-span-3 md:col-span-2 text-left pl-1">Team</div>
          ...
          <div className="col-span-3 sm:col-span-2 md:col-span-3 text-left pl-1">Cards</div>
          <div className="col-span-2 sm:col-span-4 md:col-span-4 text-left pl-2">Effects</div>
          <div className="hidden md:block col-span-1 text-center">Bonus</div>
          <div className="col-span-2 sm:col-span-2 md:col-span-2 text-right pr-2">Score</div>
        </div>
